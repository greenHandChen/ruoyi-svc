...
public final class SingleSignOutFilter extends AbstractConfigurationFilter {
    ...
    /**
     * @param casSignInProcessors
     * @return
     * @date 2021-03-24 16:49:37
     * @desc 设置登录成功processors
     **/
    public SingleSignOutFilter casSignInProcessors(List<CasSignInPostProcessor> casSignInProcessors) {
        if (!CollectionUtils.isEmpty(casSignInProcessors)) {
            casSignInProcessors.sort(Comparator.comparing(SecurityProcessor::getOrder));
            HANDLER.setCasSignInProcessors(casSignInProcessors);
        }
        return this;
    }

    /**
     * @param casSignOutProcessors
     * @return
     * @date 2021-03-24 16:49:37
     * @desc 设置登出成功processors
     **/
    public SingleSignOutFilter casSignOutProcessors(List<CasSignOutPostProcessor> casSignOutProcessors) {
        if (!CollectionUtils.isEmpty(casSignOutProcessors)) {
            casSignOutProcessors.sort(Comparator.comparing(SecurityProcessor::getOrder));
            HANDLER.setCasSignOutProcessors(casSignOutProcessors);
        }
        return this;
    }
